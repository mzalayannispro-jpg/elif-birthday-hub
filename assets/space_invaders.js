window.initSpaceInvaders = function(container) {
    if (!container.innerHTML.includes('gameCanvas')) {
        ...
        <div id="game-container">
            <div id="ui-layer">
                <header style="padding:10px 20px; display:flex; justify-content:space-between; z-index:10;">
                    <div class="score-board">LIVES: <span id="si-lives">5</span> | SCORE: <span id="si-score">0</span></div>
                    <button class="back-btn" style="padding:5px 15px; font-size:14px; margin:0;" onclick="showLandingPage(); window.cancelAnimationFrame(window.siReqId);">Quitter</button>
                </header>
                ...
                <div id="game-over-screen" class="screen">
                    <img src="lose.webp" alt="Defeat" style="max-height:250px; border-radius:12px; margin-bottom: 20px; box-shadow: 0 0 20px rgba(255,0,0,0.5);">
                    <h2>MISSION FAILED</h2>
                    <p>Final Score: <span id="final-score">0</span></p>
                    <button id="restart-btn">TRY AGAIN</button>
                    <button style="margin-top:20px; background:transparent; border:1px solid #fff;" onclick="showLandingPage()">RETOUR MENU</button>
                </div>
                
                <div id="victory-screen" class="screen">
                    <img src="win.webp" alt="Victory" style="max-height:250px; border-radius:12px; margin-bottom: 20px; box-shadow: 0 0 20px rgba(0,255,0,0.5);">
                    <h2 style="font-size:36px; text-align:center;">You Won (my heart babylovebutterfly)</h2>
    ...
    realBaklavaImg.src = 'real_baklava.png';
    ...
    function initAliens() {
        ...
        if (globalData.assets) {
            let lvlKey = level > 4 ? '1' : level.toString(); // wrap or use '1' if above 4, or better, allStickers
            if (level > 4) {
                 potentialImages = globalData.assets.allStickers;
            } else {
                 potentialImages = globalData.assets.levels[lvlKey] || globalData.assets.allStickers;
            }
        }
        
        // Fallback array if no assets
        if (!potentialImages || potentialImages.length === 0) potentialImages = ['real_baklava.png'];
    ...
    function showScreen(screen) {
        document.querySelectorAll('#module-space-invaders .screen').forEach(s => s.classList.remove('active'));
