...
with open('games/layer2/mario.js', 'r', encoding='utf-8') as f:
    code = f.read()
...
new_state_vars = state_vars_start + '''
...
# Remove old state
old_state_start = code.find('    // Fin du niveau')
old_state_end = code.find('    // Génération d\\'ennemis')

code = code[:code.find(state_vars_start)] + new_state_vars + code[old_state_end:]
...
new_player_mov = '''        let activeSpeed = player.state === 'yoshi' ? player.speed * 1.5 : player.speed;
...
code = re.sub(
    r'        if \(keys\.left\) \{ player\.vx = -player\.speed;.*?\n.*?\n.*?\n.*?\n        if \(keys\.up && player\.grounded\) \{.*?player\.vy = player\.jumpForce;',
    new_player_mov,
    code,
    flags=re.DOTALL
)
...
win_logic_old = '''            ctx.fillText("+1000 Points !", canvas.width/2, canvas.height/2 + 40);
...
win_logic_new = '''            ctx.fillText("+250 Points !", canvas.width/2, canvas.height/2 + 40);
            ...
            if (winFrame < 180) {
                ...
                if (currentLevel < LEVELS.length) {
                    ...
                    alert("Félicitations ! Tu as terminé le tour du monde de Super Elif !");
                    if (window.hideGame) window.hideGame();
                }
            }'''
...
with open('games/layer2/mario.js', 'w', encoding='utf-8') as f:
    f.write(code)
